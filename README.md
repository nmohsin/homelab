...
- **Native vs Docker** — Jellyfin, Sonarr, Radarr, Prowlarr, Bazarr, Paperless-ngx, and Nextcloud use native NixOS services (they have first-class NixOS modules). qBittorrent, FlareSolverr, Recyclarr, Homepage, and Uptime Kuma run as Docker containers (no native modules, or the container approach is simpler).
