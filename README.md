...
Port numbers are defined in `flake.nix` as `specialArgs.ports`. Internal-only scrape targets (node_exporter :9100, exportarr :9707-9711, cAdvisor :9800) are not listed — they are localhost-only and have no firewall rules.
...
secrets/
  ...
  arr-api-keys.yaml         # Encrypted Sonarr/Radarr/Prowlarr/Bazarr/Readarr API keys for Recyclarr and exportarr (sops-encrypted, yaml format)
docs/
...
**Prometheus** — `http://moyfii.tail083295.ts.net:9090`. Scrapes node_exporter (system metrics + ZFS), exportarr instances (Sonarr/Radarr/Readarr/Prowlarr/Bazarr queue metrics), and cAdvisor (Docker container metrics) every 1 minute.
