...
- Homepage config in `/etc/homepage/` is managed by Nix — edit `modules/homepage.nix` to change services, don't expect UI-driven config changes to persist. The volume is mounted writable (Homepage needs to write `logs/` there), but the Nix-managed files are symlinks into the read-only Nix store so they can't be overwritten.
